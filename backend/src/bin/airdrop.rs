...
fn main() {
    ...
    let airdrop_reciever_public_key = env::var("PUBLIC_KEY").expect("Public key env variable not found");
    let pubkey = airdrop_reciever_public_key.parse::<Pubkey>().expect("Invalid Public Key, Conversion Failed");

    let client = RpcClient::new(solana_devnet);

    let signature = client.request_airdrop(&pubkey, LAMPORTS_PER_SOL*5).unwrap();
