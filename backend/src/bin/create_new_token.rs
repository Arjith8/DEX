...
use solana_sdk::{
    commitment_config::CommitmentConfig, program_pack::Pack, signer::Signer,
...
fn main() {
    ...
    let solana_devnet =
        env::var("SOLANA_DEVNET").unwrap_or_else(|_| "https://api.devnet.solana.com".to_string());
    ...
    let min_balance_for_rent_excemption = client
    ...
    let keypair_path = args()
        .nth(1)
        .expect("Please provide the path to the mint keypair file");
    ...
    let token_min_instructions = initialize_mint(
        &spl_token_2022::id(),
        &mint_keypair.pubkey(),
        &fee_payer.pubkey(),
